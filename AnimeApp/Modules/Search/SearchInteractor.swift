...
final class SearchInteractor: SearchBusinessLogic, SearchDataStore {
    ...
    /// Returns `animes` with duplicate `malID`s removed, keeping the first
    /// occurrence and preserving order.
    private static func deduplicatedByID(_ animes: [Anime]) -> [Anime] {
        var seen = Set<Int>()
        return animes.filter { seen.insert($0.malID).inserted }
    }
    ...
    private func loadPage(reset: Bool) async {
        ...
        do {
            ...
            // Jikan can return the same anime on more than one page (and even twice
            // within a page). The View's diffable data source traps on duplicate
            // item identifiers, so collapse to the first occurrence of each malID.
            results = Self.deduplicatedByID(results)
