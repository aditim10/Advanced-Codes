...
final class HomeViewController: UIViewController, HomeDisplayLogic {
    ...
    private lazy var bannerContainer: BannerView = {
        ...
        banner.frame = CGRect(x: 0, y: 0, width: view.bounds.width,
                              height: Layout.bannerHeight(forAvailableHeight: view.bounds.height))
    ...
    /// The banner is a `tableHeaderView`, which UIKit does NOT auto-resize. Keep
    /// its width in sync with the table and its height adaptive to the device, and
    ...
    private func sizeTableHeaderToFit() {
        ...
        bannerContainer.frame = CGRect(x: 0, y: 0, width: width, height: height)
        tableView.tableHeaderView = bannerContainer
    }

    /// When the size class changes (e.g. iPad multitasking, or a rotation that
    /// flips compact↔regular), poster cards change size, so re-query the row
    /// heights. `performBatchUpdates(nil)` re-asks `heightForRowAt` without
    /// touching the diffable snapshot (a plain `reloadData()` would desync it).
...
extension HomeViewController: UITableViewDelegate {
    ...
    func tableView(_ tv: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        Layout.sectionRowHeight(for: traitCollection)
    }
