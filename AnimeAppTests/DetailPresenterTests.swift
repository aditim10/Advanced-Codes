...
final class DetailPresenterTests: XCTestCase {
    ...
    func testPresentDetailUsesSynopsisFallbackWhenMissing() {
        ...
        let noSynopsis = Anime(
            ...
            year: base.year, images: base.images, genres: base.genres, trailer: base.trailer)
    ...
    func testPresentDetailSurfacesTrailerYouTubeID() {
        let (presenter, display) = makePresenter()
        // The fixture mirrors the real Jikan shape: youtube_id is null, but the id
        // lives in embed_url — we should still extract it.
        let anime = Fixture.anime("anime_detail")
        presenter.presentDetail(response: Detail.Load.Response(anime: anime, characters: []))
        XCTAssertEqual(display.lastViewModel?.trailerYouTubeID, "Vt_3c8BgxV4")
    }

    func testPresentDetailTrailerIsNilWhenAbsent() {
        let (presenter, display) = makePresenter()
        let anime = Sample.anime(id: 1)             // Sample has no trailer
        presenter.presentDetail(response: Detail.Load.Response(anime: anime, characters: []))
        XCTAssertNil(display.lastViewModel?.trailerYouTubeID)
    }
