...
final class AnalyticsManagerTests: XCTestCase {
    ...
    override func setUp() {
        super.setUp()
        // Isolated instance (not the shared singleton). Synchronous delivery so
        // assertions can run right after emitting.
    ...
    func testPublishForwardsToRegisteredProvider() {
        ...
        bus.emit(AppLifecycleEvent.appLaunch)
        bus.emit(LoginAnalyticsEvent.success(userID: "anon-123"))
    ...
    func testPublishFansOutToMultipleProviders() {
        ...
        bus.emit(HomeAnalyticsEvent.loadSuccess(sectionCount: 3))
    ...
    func testEventCarriesPayload() {
        ...
        bus.emit(SearchAnalyticsEvent.resultSuccess(query: "naruto", count: 7))
