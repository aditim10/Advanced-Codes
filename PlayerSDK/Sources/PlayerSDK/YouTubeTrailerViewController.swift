...
//  PlayerSDK
//
//  Plays a *real* YouTube trailer the same way MyAnimeList does: a `WKWebView`
//  running YouTube's official IFrame Player API. YouTube serves protected,
//  ad-supported streams that AVPlayer cannot open directly, so the official embed
//  is the correct (and most reliable) way to show real trailer content.
//
...
public final class YouTubeTrailerViewController: UIViewController {
    ...
    fileprivate func handleBridgeMessage(_ body: String) {
        ...
        if body.hasPrefix("error:"), let code = Int(body.dropFirst("error:".count)) {
            ...
            spinner.stopAnimating()
            let error = NSError(domain: "PlayerSDK.YouTube", code: code,
